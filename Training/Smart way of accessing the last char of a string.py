...
# We wrap the string if is too big and we continue in the next line. It is better practice

ipAddress = input(input_prompt)
if ipAddress[-1] != '.': # Checking first if it is not ending in '.'
    ipAddress += '.'
...
# We actually overcome both issues, not needed to initialise the character and also
# adding a '.' at the end of the IP so that we perform one less check.
