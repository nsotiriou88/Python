...
def coordinateFunc2(ancMat, anc2DPos, tagMat, tagPos, actualPositions):
    ...
    bestTri = min(i for i in arVec if i > 0)
    bestInd = list(arVec).index(bestTri)
    

    # And we also create vectors of its cyclic permutations
    refTri1 = list(ancIts[bestInd])
    ...
    for j in pointsToFind:
        ...
        if trueDist - dist1 < distTol:
            ...
            soln3 = np.array([[np.nan],[np.nan]])

        # combine all the solutions and take the mean (ignoring nans)
        allSolns = np.column_stack((soln1,soln2,soln3))
        meanSoln = np.array([np.nanmean(allSolns[0, :]), np.nanmean(allSolns[1, :])])
    ...
    for j in range(0,3):
        ...
        tempTagPos[j, :] = qrLocate.qrLocate(tagMat[j, :], anc2DPos, refTri1, 5, ancNum, ancMat)
    ...
    # Just using this format to avoid lots of commented out code
    if 1:
        #plt.subplot(1, 2, 1)
